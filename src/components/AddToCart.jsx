...
export default function AddToCart() {
  ...
  const Increment = (item) => {
    const updatedCartItems = login.cartItems.map((cart) =>
      cart.id === item.id ? { ...cart, amount: cart.amount + 1 } : cart
    );
    dispach(setLogin({ ...login, cartItems: updatedCartItems }));
  ...
  const handleDelete = (id) => {
    setLogin((prevLogin) => ({
      ...prevLogin,
      cartItems: prevLogin.cartItems.filter((data) => data.id != id),
    }));
  };
                    ...
                    <div className="col-lg-7">
                      ...
                      <div className="d-flex justify-content-between align-items-center mb-4">
                        <div>
                          ...
                          <p className="mb-0">
                            You have {login.cartItems.length} items in your cart
                          </p>
                        </div>
                      </div>

                      {login &&
                        login.cartItems.map((cart) => {
                          return (
